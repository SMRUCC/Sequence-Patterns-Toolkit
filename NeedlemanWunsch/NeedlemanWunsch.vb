...
Public Class NeedlemanWunsch
    Inherits NeedlemanWunschArguments
    ...
    Protected Friend Sub traceback(s1 As Stack(Of Char?), s2 As Stack(Of Char?), i As Integer, j As Integer)
        ...
        Select Case direction
            ...
            Case 1 ' upper cell
                s1.Push("-"c)
                s2.Push(Me.Sequence2.Chars(i - 2))
                Me.traceback(s1, s2, i - 1, j)
            Case 2 ' upperLeft cell
                s1.Push(Me.Sequence1.Chars(j - 2))
                s2.Push(Me.Sequence2.Chars(i - 2))
    ...
    Public Sub compute()
        ...
        Dim ___sequence1 As String = Me.Sequence1
        Dim ___sequence2 As String = Me.Sequence2
        ...
        For i As Integer = 1 To rows - 1
            For j As Integer = 1 To columns - 1
                Dim a As Integer = matrix(i - 1)(j - 1) + Me.match(___sequence1.Chars(j - 1), ___sequence2.Chars(i - 1))
                Dim b As Integer = matrix(i)(j - 1) - Me.GapPenalty
                Dim c As Integer = matrix(i - 1)(j) - Me.GapPenalty
                Dim max As Integer = Me.max(a, b, c)
